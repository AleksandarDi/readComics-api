...
class SavedComics extends Component{
    ...
    componentWillMount(){
        getUserSaved(sessionStorage.getItem("currentUser_id")).then((data) => {
            console.log(data);
            if(data.length > 0) {
                this.setState({
                    favorites: data,
    ...
    removeFromSaved = (comic) =>{
        removeSavedFromUser(sessionStorage.getItem("currentUser_id"), comic).then(
    ...
    render(){
        ...
        if(this.state.favorites !== null) {
            var comics = this.state.favorites.map((comic, i) => (
                <Table.Row key={i}>
                    ...
                    <Table.Cell>
                        <Button
                            onClick={this.removeFromSaved.bind(this, comic.id)}
                            basic color='red'
                            animated='vertical'
                            hidden={this.state.buttonDisabled}>
        ...
        return(
            <div>
                ...
                {!this.state.showTable && <h4 className="text-muted text-center m-5">You haven't saved any comics.</h4>}
            </div>
        );
    }
